...
def _story_core_line(
    ...
    dm_core = {
        "甲": "곧게 뻗은 나무처럼 강직하고 추진력이 강한",
        "乙": "넝쿨처럼 유연하고 어떤 환경에서도 살아남는",
        "丙": "태양처럼 밝고 주변을 환하게 만드는",
        "丁": "촛불처럼 섬세하고 따뜻한 빛을 내는",
        "戊": "넓은 대지처럼 묵직하고 포용력 깊은",
        "己": "기름진 논밭처럼 실속 있고 현실적인",
        "庚": "단단한 바위처럼 강하고 원칙적인",
        "辛": "보석처럼 섬세하고 완벽을 추구하는",
        "壬": "큰 강처럼 넓고 지혜로운",
        "癸": "빗물처럼 섬세하고 감성 풍부한",
    }
    dm_desc = dm_core.get(day_master, "독특한 기운의")
...
def _story_health_lifetime(
    ...
    weak_desc: List[str] = []
    for e in weak_e[:2]:
        organ = organ_detail.get(e, e)
        weak_desc.append(
            f"{e}({organ}) 부분이 선천적으로 약할 수 있으니 정기 검진을 우선하세요. "
            f"이 사주는 {e} 오행이 약할수록 계절 변화 때 몸이 먼저 반응하기 쉽습니다."
        )

    strong_desc: List[str] = []
    for e in strong_e[:2]:
        organ = organ_detail.get(e, e)
        strong_desc.append(
            f"{e}({organ}) 계열은 과로 시 가장 먼저 신호가 옵니다. "
            f"이 사주는 {e} 오행이 과할수록 쉬는 리듬을 넣지 않으면 피로가 누적되기 쉽습니다."
        )

    all_bodies = weak_desc + strong_desc
    if not all_bodies:
        all_bodies = [
            "종합 검진으로 개인별 우선순위를 확인하세요. "
            "이 사주는 체질 차이가 크므로 ‘나만의 약한 축’을 숫자로 잡아두는 것이 장기적으로 유리합니다."
        ]

    age_notes = {
        "유소년": (
            "성장기에는 과다 오행 장부의 피로와 알레르기를 의식하세요. "
            "이 사주는 성장기에 면역·피부 반응이 민감해질 수 있어 생활 환경을 안정적으로 유지하는 것이 중요합니다."
        ),
        "청년": (
            "20~30대는 과로·불면이 가장 위험합니다. 위장과 근골격 피로가 쌓이기 쉬우니 수면과 식사 리듬을 꼭 지키세요. "
            "이 사주는 바쁠수록 ‘최소 휴식’이라도 고정 시간으로 넣어야 회복 속도가 따라옵니다."
        ),
        "중년": (
            "40~50대에는 충이 있는 지지 장부가 만성 질환의 전조가 됩니다. 정기 검진 간격을 2년에서 1년으로 줄이는 것을 권장합니다. "
            "이 사주는 몸의 신호를 일찍 받아들일수록 치료 비용과 시간을 크게 줄일 수 있습니다."
        ),
        "시니어": (
            "60대 이후에는 낙상 예방과 순환·혈압 관리가 장수의 분기점입니다. 결핍 오행 장부 보양식과 가벼운 유산소 운동이 도움됩니다. "
            "이 사주는 ‘천천히 자주’ 움직이는 습관이 혈관과 관절을 동시에 지켜 줍니다."
        ),
    }
    ...
    if _has_sinsal_lines(sinsal, "백호살") or _has_sinsal_lines(sinsal, "양인살"):
        longevity += (
            " 백호·양인은 급성 사고·수술 변수를 의식합니다. "
            "이 사주는 안전 거리·운전·날카로운 환경만 조심해도 리스크가 크게 줄어듭니다."
        )

    yong_el = yong.get("용신_오행", "") or ""

    return {
        "선천_취약_축": all_bodies[:3],
        "나이대별_주의": age_notes,
        "장수_가능성": longevity,
        "건강_유지_조언": [
